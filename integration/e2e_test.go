...
func TestHTTPAndGRPCEndToEnd(t *testing.T) {
	...
	client := meteringv1.NewMeteringServiceClient(connection)
	created, err := client.CreateMeter(pskCtx, &meteringv1.CreateMeterRequest{Code: "api.calls", Name: "API calls", Unit: "request", Aggregation: "sum", DimensionKeys: []string{"endpoint"}})
