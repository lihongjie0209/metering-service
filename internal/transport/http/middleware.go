...
import (
	...
	appLimit "github.com/lihongjie0209/metering-service/internal/ratelimit"
...
func Authorization(enabled bool, authorizer platformauthz.Authorizer, logger *slog.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		requirement, protected := meteringHTTPRequirement(c.FullPath())
...
func meteringHTTPRequirement(route string) (platformauthz.Requirement, bool) {
...
func Authentication(service *auth.Service, logger *slog.Logger, cfg config.Auth) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		if cfg.PSK.Enabled && auth.MatchesAny(c.FullPath(), cfg.PSK.HTTPPaths) {
			...
			c.Set("subject", "psk")
			ctx := platformprincipal.WithContext(c.Request.Context(), platformprincipal.Principal{ID: "metering-service:psk", Type: platformprincipal.TypeServiceAccount})
