...
import (
	...
	docs "github.com/lihongjie0209/metering-service/docs"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, handler *Handler, authService *auth.Service, authorizer platformauthz.Authorizer, limiter *ratelimit.Limiter, metrics *observability.Metrics, tracing *observability.Tracing, logger *slog.Logger) (*http.Server, error) {
	...
	api.POST("/meters/update", handler.UpdateMeter)
	api.POST("/meters/get", handler.GetMeter)
	api.POST("/meters/list", handler.ListMeters)
	api.POST("/usage/record", handler.RecordUsage)
	api.POST("/usage/query", handler.QueryUsage)
	api.POST("/usage/adjust", handler.AdjustUsage)
