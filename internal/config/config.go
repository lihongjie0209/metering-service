...
func setDefaults(v *viper.Viper) {
	v.SetDefault("app.name", "metering-service")
	...
	v.SetDefault("database.schema", "metering")
	...
	v.SetDefault("jwt.issuer", "metering-service")
	...
	v.SetDefault("migration.table", "metering_schema_migrations")
...
func (c Config) Validate() error {
	...
	if c.App.Env == "production" && (c.Auth.JWKSURL == "" || c.Auth.Issuer == "" || c.Auth.Audience == "") {
		return errors.New("production authentication requires identity JWKS URL, issuer, and metering-service audience")
	}
	...
	if c.Database.Enabled {
		if _, ok := c.Outbound.GRPC["application"]; !ok {
			return errors.New("metering application scope requires outbound.grpc.application")
		}
	}
